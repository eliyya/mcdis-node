...
const processNameSchema = z.string().min(1).max(40).regex(/^[\w .-]+$/u);
...
export type ManagedProcessConfig = z.infer<typeof processSettingsSchema> & {
  name: string;
  prefix: string;
  cwd: string;
};
...
export type AppConfig = {
  ...
  processes: Record<string, ManagedProcessConfig>;
};
...
export async function loadConfig(configPath: string): Promise<AppConfig> {
  ...
  const processes: Record<string, ManagedProcessConfig> = {};
