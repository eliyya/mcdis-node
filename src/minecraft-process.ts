...
import type { ManagedProcessConfig } from './config.js';
...
export class MinecraftProcess extends EventEmitter {
  ...
  async handleThreadCommand(messageOrCommand: Message | string): Promise<void> {
    const rawCommand = typeof messageOrCommand === 'string' ? messageOrCommand : messageOrCommand.content;
    ...
    } else if (typeof messageOrCommand !== 'string' && await this.pluginManager.notifyDiscordMessage(messageOrCommand, commandWithoutPrefix)) {
      return;
