...
export class ProcessPluginManager {
  ...
  async load({ reload = false }: { reload?: boolean } = {}): Promise<void> {
    ...
    for (const source of sources) {
      try {
        ...
        this.process.sendManagerLog(`Unable to import plugin ${source.name}: ${formatError(error)}`);
      }
    }
  }
  ...
  async unload(): Promise<void> {
    while (this.loadedPlugins.length > 0) {
      ...
      try {
        ...
        this.process.emit('plugin:unloaded', loadedPlugin.plugin, loadedPlugin.file, this.process);
      }

      if (unloadError) {
        this.process.sendManagerLog(`Plugin unload failed ${path.basename(loadedPlugin.file)}: ${formatError(unloadError)}`);
      }
    }
  }
  ...
  async notifyConsoleLine(line: string): Promise<void> {
    for (const { plugin, file } of this.loadedPlugins) {
      try {
        ...
        this.process.sendManagerLog(`Plugin console hook failed ${path.basename(file)}: ${formatError(error)}`);
      }
    }
  }
  ...
  async notifyDiscordMessage(message: Message, command: string): Promise<boolean> {
    ...
    for (const { plugin, file } of this.loadedPlugins) {
      try {
        ...
        this.process.sendManagerLog(`Plugin Discord hook failed ${path.basename(file)}: ${formatError(error)}`);
      }
    }
  ...
  private async callHook(hookName: 'onStart' | 'onStop'): Promise<void> {
    for (const { plugin, file } of this.loadedPlugins) {
      try {
        ...
        this.process.sendManagerLog(`Plugin ${hookName} failed ${path.basename(file)}: ${formatError(error)}`);
      }
    }
  }
  ...
  private async getPluginSources(): Promise<RegisteredPluginSource[]> {
    ...
    for (const entry of entries) {
      ...
      if (fileStat?.isFile() && pluginExtensions.has(path.extname(entry))) {
        ...
        const directoryEntrypoint = await this.resolveDirectoryEntrypoint(file).catch((error) => {
          this.process.sendManagerLog(`Unable to resolve plugin directory ${entry}: ${formatError(error)}`);
          return undefined;
        });
  ...
  private async resolvePackageEntrypoint(directory: string): Promise<string | undefined> {
    ...
    if (relativeCandidate.startsWith('..') || path.isAbsolute(relativeCandidate)) {
      throw new Error(`${path.basename(directory)} package.json entrypoint must stay inside the plugin directory.`);
    }

    const candidateStat = await stat(candidate).catch(() => undefined);
    if (!candidateStat?.isFile() || !pluginExtensions.has(path.extname(candidate))) {
      throw new Error(`${path.basename(directory)} package.json entrypoint must be a .js, .mjs, .cjs or .ts file.`);
    }
  ...
  private async loadPluginSource(source: RegisteredPluginSource): Promise<Plugin> {
    ...
    if (!(plugin instanceof Plugin)) {
      throw new Error('Registered plugin factory must return an instance that extends Plugin.');
    }
...
async function readPluginPackageJson(packageJsonPath: string): Promise<PluginPackageJson | undefined> {
