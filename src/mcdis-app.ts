...
export class McDisApp extends EventEmitter {
  ...
  constructor(readonly config: AppConfig) {
    ...
    for (const processConfig of Object.values(config.processes)) {
      const process = new MinecraftProcess(processConfig, this, this.threadManager, this.queue);
  ...
  async registerProcessPlugin(options: ProcessPluginRegistrationOptions): Promise<() => Promise<void>> {
  ...
  async registerProcessPluginForMany(options: MultiProcessPluginRegistrationOptions): Promise<() => Promise<void>> {
    ...
    for (const process of targetProcesses) {
      unregisterCallbacks.push(await process.registerPluginSource(this.createPluginSource(options)));
    }
  ...
  private createPluginSource(options: Omit<ProcessPluginRegistrationOptions, 'processName'>): RegisteredPluginSource {
    if (options.file) {
  ...
  private async handleThreadMessage(message: Message): Promise<void> {
    ...
    if (channel.type !== ChannelType.PublicThread || channel.parentId !== this.config.discord.panelChannelId) {
      return;
    }
  ...
  private async handlePanelCommand(message: Message): Promise<void> {
    ...
    if (command === 'status' && targetName.length === 0) {
      await message.reply([...this.processes.values()].map((item) => item.getStatusLine()).join('\n') || 'No processes configured.');
      return;
    }
