...
export class ModManager {
  ...
  async load({ reload = false }: { reload?: boolean } = {}): Promise<void> {
    ...
    for (const entrypoint of entrypoints) {
      try {
        ...
        await this.app.sendManagerLog(`Unable to import mod ${entrypoint.name}: ${formatError(error)}`);
      }
    }
  }
  ...
  async unload(): Promise<void> {
    while (this.loadedMods.length > 0) {
      ...
      if (unloadError) {
        await this.app.sendManagerLog(`Mod unload failed ${path.basename(loadedMod.file)}: ${formatError(unloadError)}`);
      }
    }
  }
  ...
  async notifyDiscordMessage(message: Message): Promise<boolean> {
    ...
    for (const { mod, file } of this.loadedMods) {
      try {
        ...
        await this.app.sendManagerLog(`Mod Discord hook failed ${path.basename(file)}: ${formatError(error)}`);
      }
    }
  ...
  private async callHook(hookName: 'onReady' | 'onShutdown'): Promise<void> {
    for (const { mod, file } of this.loadedMods) {
      try {
        ...
        await this.app.sendManagerLog(`Mod ${hookName} failed ${path.basename(file)}: ${formatError(error)}`);
      }
    }
  }
    ...
    for (const entry of entries) {
      ...
      if (fileStat?.isFile() && modExtensions.has(path.extname(entry))) {
        ...
        const directoryEntrypoint = await this.resolveDirectoryEntrypoint(file).catch((error) => {
          void this.app.sendManagerLog(`Unable to resolve mod directory ${entry}: ${formatError(error)}`);
          return undefined;
        });
  ...
  private async resolvePackageEntrypoint(directory: string): Promise<string | undefined> {
    ...
    if (relativeCandidate.startsWith('..') || path.isAbsolute(relativeCandidate)) {
      throw new Error(`${path.basename(directory)} package.json entrypoint must stay inside the mod directory.`);
    }
